...
(function () {
  ...
  window.addEventListener('error', function (event) {
    ...
    // Script/resource load failures: event.error is often null; use event.target for diagnostics
    if ((!msg || msg === '(no message)') && event.target && !event.error) {
      try {
        if (event.target.tagName === 'SCRIPT') {
          var href = event.target.src || event.target.getAttribute('src') || '';
          if (href) msg = 'Script load failed: ' + href;
        } else if (event.target.tagName === 'LINK') {
          var h = event.target.href || event.target.getAttribute('href') || '';
          if (h) msg = 'Resource load failed: ' + h;
        }
      } catch { /* ignore */ }
    }
